  ...
  <div v-else class="divide-y divide-default rounded-lg border border-default overflow-hidden">
    ...
    >
      <div class="shrink-0 w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center">
        <FontAwesomeIcon icon="music" class="text-primary" />
      </div>
      ...
      <UBadge :color="track.category === 'liturgy' ? 'info' : 'neutral'" variant="subtle">
      ...
      <div class="flex gap-1 shrink-0">
        ...
        <UButton variant="ghost" size="sm" @click="$emit('edit', track)">
          <FontAwesomeIcon icon="pen" />
        </UButton>
        <UButton variant="ghost" size="sm" color="error" @click="$emit('delete', track)">

  ...
  <UModal :open="open" @update:open="emit('update:open', $event)">
    <template #content>
      <UCard class="max-h-[70vh] flex flex-col">
        <template #header>
          <h3 class="font-semibold">Track auswählen</h3>
        </template>

        <UInput v-model="search" placeholder="Suchen…" icon="i-lucide-search" class="mb-4" />
        ...
        <div class="overflow-y-auto flex-1 divide-y divide-default">
          ...
          >
            <FontAwesomeIcon icon="music" class="text-muted" />
            <div class="min-w-0">
              <p class="font-medium truncate">{{ track.title }}</p>
              <p class="text-sm text-muted truncate">{{ track.composer || 'Unbekannt' }}</p>
            </div>
          </button>
          <p v-if="!filtered.length" class="text-center text-muted py-8">Keine Tracks gefunden</p>
        </div>
      </UCard>
    </template>
  </UModal>

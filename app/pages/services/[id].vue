...
<template>
  ...
  <div v-else-if="service" class="space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        <UButton to="/services" variant="ghost" size="sm" class="mb-2">← Zurück</UButton>
        <h1 class="text-2xl font-semibold">{{ service.name }}</h1>
        <p class="text-muted">{{ formatServiceDate(service.serviceDate) }}</p>
        <p v-if="saveState === 'saving'" class="text-sm text-muted mt-1">Speichert…</p>
        <p v-else-if="saveState === 'saved'" class="text-sm text-success mt-1">Gespeichert</p>
        <p v-else-if="saveState === 'error'" class="text-sm text-error mt-1">Fehler beim Speichern</p>
        <UBadge v-if="isCached" color="success" variant="subtle" class="mt-2">Offline bereit</UBadge>
      </div>
      <div class="flex gap-2">
        <UButton variant="outline" :loading="preparing" @click="prepareOffline">
          <FontAwesomeIcon icon="cloud-arrow-down" class="mr-2" />
          Offline vorbereiten
        </UButton>
        <UButton :to="`/playback/${id}`" color="primary">Zur Wiedergabe</UButton>
      </div>
    </div>

    <UProgress v-if="preparing" :model-value="progress" :max="100" />
    ...
    <LibraryTrackPicker v-model:open="pickerOpen" category-filter="hymn" @pick="onTrackSelected" />
  </div>
</template>
